...
<?php
function count_complete_decks($cards_dropped) {
    $suits = ['D', 'H', 'S', 'K'];
    $values = ['2', '3', '4', '5', '6', '7', '8', '9', '0', 'J', 'Q', 'K', 'A'];
    $standard_deck = [];
    foreach ($suits as $suit) {
        foreach ($values as $value) {
            array_push($standard_deck, $suit . $value);
        }
    }
    $card_count = array_fill_keys($standard_deck, 0);
    foreach ($cards_dropped as $card) {
        if (in_array($card, $standard_deck)) {
            $card_count[$card]++;
        }
    }
    $complete_decks = min($card_count);
...
echo "Test case 1: ";
echo "Answer :---------------------";
echo count_complete_decks($test_cards_1);

echo "Test case 2: ";
echo "Answer :---------------------";
echo count_complete_decks($test_cards_2);
